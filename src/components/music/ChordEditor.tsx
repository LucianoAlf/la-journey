...
export type ChordInstrument = 'guitar' | 'ukulele' | 'bass'
...
const INSTRUMENT_CONFIGS: Record<ChordInstrument, InstrumentConfig> = {
  guitar: { strings: 6, names: ['E', 'A', 'D', 'G', 'B', 'e'], neckW: 260, neckH: 440, prevW: 160, prevH: 210 },
  ukulele: { strings: 4, names: ['G', 'C', 'E', 'A'], neckW: 200, neckH: 440, prevW: 130, prevH: 210 },
  bass: { strings: 4, names: ['E', 'A', 'D', 'G'], neckW: 200, neckH: 440, prevW: 130, prevH: 210 },
}

function getConfig(instrument?: ChordInstrument): InstrumentConfig {
  return INSTRUMENT_CONFIGS[instrument ?? 'guitar']
}

/** Quantas cordas tem o instrumento */
export function getStringCount(instrument?: ChordInstrument): number {
  return getConfig(instrument).strings
}
...
export function createEmptyState(stringCount = 6): ChordEditorState {
  return { dots: [], openMuted: Array(stringCount).fill(null), barres: [] }
}

/** Converte positions do banco → estado do editor */
export function positionsToState(positions: ChordPositions, startFret = 1, instrument?: ChordInstrument): ChordEditorState {
  const S = getStringCount(instrument)
...
export function stateToPositions(state: ChordEditorState, startFret = 1, instrument?: ChordInstrument): ChordPositions {
  const S = getStringCount(instrument)
  ...
  state.dots.forEach(d => {
    const sv = S - d.s, fret = d.f + startFret
...
// ── Layout helpers (parametrizados) ─────────────────────────────────────

function stringSpacing(cfg: InstrumentConfig) { return (cfg.neckW - PL - PR) / (cfg.strings - 1) }
function fretSpacing(cfg: InstrumentConfig) { return (cfg.neckH - PT - NUT_H - PB) / F }
function stringX(s: number, cfg: InstrumentConfig) { return PL + s * stringSpacing(cfg) }
function fretY(f: number, cfg: InstrumentConfig) { return PT + NUT_H + f * fretSpacing(cfg) }
function cellY(f: number, cfg: InstrumentConfig) { return fretY(f, cfg) + fretSpacing(cfg) / 2 }

// ── Hit testing ─────────────────────────────────────────────────────────

function findString(mx: number, cfg: InstrumentConfig) {
  for (let s = 0; s < cfg.strings; s++) if (Math.abs(mx - stringX(s, cfg)) < stringSpacing(cfg) / 2) return s
  return null
}

function findFret(my: number, cfg: InstrumentConfig) {
  for (let f = 0; f < F; f++) if (Math.abs(my - cellY(f, cfg)) < fretSpacing(cfg) / 2) return f
  return null
}

function findDot(dots: Dot[], mx: number, my: number, cfg: InstrumentConfig) {
  for (let i = dots.length - 1; i >= 0; i--) {
    const d = dots[i]
    const dx = mx - stringX(d.s, cfg), dy = my - cellY(d.f, cfg)
    if (dx * dx + dy * dy < 16 * 16) return i
  }
  return null
}

function findBarre(barres: Barre[], mx: number, my: number, cfg: InstrumentConfig) {
  for (let i = barres.length - 1; i >= 0; i--) {
    const b = barres[i]
    const mn = Math.min(b.from, b.to), mxB = Math.max(b.from, b.to)
    const y = cellY(b.fret, cfg)
    if (my > y - 14 && my < y + 14 && mx > stringX(mn, cfg) - 14 && mx < stringX(mxB, cfg) + 14) return i
  }
  return null
}
...
function drawNeck(
  ...
  const c = ctx
  const S = cfg.strings
  c.clearRect(0, 0, cfg.neckW, cfg.neckH)
  ...
  for (let i = 0; i < S; i++) {
    ...
    c.fillText(cfg.names[i], stringX(i, cfg), 12)
  }

  // Open/Muted
  for (let i = 0; i < S; i++) {
    const x = stringX(i, cfg), y = 28
    if (openMuted[i] === 'open') {
  ...
  if (startFret === 1) {
    ...
    c.fillRect(PL - 2, PT, cfg.neckW - PL - PR + 4, NUT_H)
  }

  // Trastes
  for (let f = 1; f <= F; f++) {
    c.strokeStyle = fretColor; c.lineWidth = 1.5
    c.beginPath(); c.moveTo(PL - 2, fretY(f, cfg)); c.lineTo(PL + (S - 1) * stringSpacing(cfg) + 2, fretY(f, cfg)); c.stroke()
  }

  // Cordas com espessura variável
  const halfS = Math.ceil(S / 2)
  for (let i = 0; i < S; i++) {
    c.strokeStyle = i < halfS ? stringColorA : stringColorB
    c.lineWidth = Math.max(2.5 - i * (1.7 / (S - 1)), 0.8)
    c.beginPath(); c.moveTo(stringX(i, cfg), PT + NUT_H); c.lineTo(stringX(i, cfg), fretY(F, cfg)); c.stroke()
  }
  ...
  for (let f = 0; f < F; f++) c.fillText(String(f + startFret), PL - 10, cellY(f, cfg))

  // Marcadores de posição
  const markerS1 = Math.floor((S - 1) / 2), markerS2 = Math.ceil((S - 1) / 2)
  for (let f = 0; f < F; f++) {
    if (MARKERS.includes(f + startFret)) {
      c.fillStyle = markerColor
      c.beginPath(); c.arc((stringX(markerS1, cfg) + stringX(markerS2, cfg)) / 2, cellY(f, cfg), 4, 0, Math.PI * 2); c.fill()
    }
  }
  ...
  if (barreMode) {
    ...
    c.fillRect(PL - 5, PT, (S - 1) * stringSpacing(cfg) + 10, F * fretSpacing(cfg))
  }
  if (barreFirst) {
    c.strokeStyle = ACCENT; c.lineWidth = 2; c.setLineDash([4, 4])
    c.beginPath(); c.arc(stringX(barreFirst.s, cfg), cellY(barreFirst.f, cfg), 16, 0, Math.PI * 2); c.stroke()
    c.setLineDash([])
  }
  ...
  barres.forEach(b => {
    ...
    const y = cellY(b.fret, cfg), x1 = stringX(mn, cfg), x2 = stringX(mx, cfg), bh = 18, r = bh / 2
  ...
  dots.forEach((d, i) => {
    const x = stringX(d.s, cfg), y = cellY(d.f, cfg), r = 14
...
function drawPreview(
  ...
  const c = ctx
  const S = cfg.strings
  c.clearRect(0, 0, cfg.prevW, cfg.prevH)

  const pt = 22, pr = 15
  const pl = startFret === 1 ? 15 : 30
  const nutY = pt + 4, fsp = 25
  const ssp = (cfg.prevW - pl - pr) / (S - 1)
  ...
  if (startFret === 1) {
    c.fillStyle = nutColor
    c.fillRect(pl - 1, nutY, cfg.prevW - pl - pr + 2, 3)
  } else {
    // Linha fina no lugar do nut
    c.strokeStyle = fretColor; c.lineWidth = 1
    c.beginPath(); c.moveTo(pl - 1, nutY + 1); c.lineTo(cfg.prevW - pr + 1, nutY + 1); c.stroke()
  ...
  // Cordas
  for (let s = 0; s < S; s++) {
    c.strokeStyle = stringColor; c.lineWidth = Math.max(1.6 - s * (0.8 / (S - 1)), 0.8)
  ...
  for (let f = 1; f <= F; f++) {
    c.strokeStyle = fretColor; c.lineWidth = 1
    c.beginPath(); c.moveTo(pl - 1, nutY + 3 + f * fsp); c.lineTo(cfg.prevW - pr + 1, nutY + 3 + f * fsp); c.stroke()
  }
...
export function ChordEditor({ state, onChange, chordName = '', startFret = 1, instrument }: ChordEditorProps) {
  const cfg = useMemo(() => getConfig(instrument), [instrument])
  ...
  const renderAll = useCallback(() => {
    ...
    if (nCtx) drawNeck(nCtx, state.dots, state.openMuted, state.barres, startFret, selectedDot, barreMode, barreFirst, isDark, cfg)
    if (pCtx) drawPreview(pCtx, state.dots, state.openMuted, state.barres, startFret, isDark, cfg)
  }, [state, startFret, selectedDot, barreMode, barreFirst, isDark, cfg])
  ...
  const handleContextMenu = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const di = findDot(state.dots, mx, my, cfg)
    if (di !== null) {
      setSelectedDot(di)
      const d = state.dots[di]
      setTipPos({ x: stringX(d.s, cfg) - 68, y: cellY(d.f, cfg) - 46 })
      setTipVisible(true)
    } else {
      hideTooltip()
    }
  }, [state.dots, hideTooltip, cfg])
  ...
  const handleClick = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const s = findString(mx, cfg), f = findFret(my, cfg)
    ...
    if (findDot(state.dots, mx, my, cfg) !== null) return
  ...
  }, [state, onChange, barreMode, barreFirst, hideTooltip, cfg])
  ...
  const clearAll = useCallback(() => {
    ...
    setBarreFirst(null)
    hideTooltip()
    onChange(createEmptyState(cfg.strings))
  }, [onChange, hideTooltip, cfg])
      ...
      <div className="flex gap-6 items-start">
        ...
        <div className="flex flex-col items-center gap-2">
          <div className="bg-bg border border-border rounded-xl p-4 text-center">
            ...
            <canvas ref={prevRef} width={cfg.prevW} height={cfg.prevH} />
