import { Books, Copy, ListChecks, PencilSimple, Trash } from '@phosphor-icons/react'
import type { ExerciseCollection } from '@/services/exerciseCollectionService'
import { Badge } from '@/components/ui/badge'
import { Button } from '@/components/ui/button'

interface ExerciseNotebookCardProps {
  notebook: ExerciseCollection & { exerciseCount: number }
  onOpen: (notebook: ExerciseCollection) => void
  onEdit: (notebook: ExerciseCollection) => void
  onDelete: (notebook: ExerciseCollection) => void
...
export function ExerciseNotebookCard({
  notebook,
  onOpen,
  onEdit,
  onDelete,
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        <span className="text-[11px] text-text3 flex items-center gap-1.5">
          <ListChecks size={12} />
          {notebook.exerciseCount} exercício{notebook.exerciseCount !== 1 ? 's' : ''}
        </span>
      </div>
